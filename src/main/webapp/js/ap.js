    ...
    var d = function () {
        if (M.ie && M.win) {
            window.attachEvent("onunload", function () {
                ...
                M = null;
                for (var X in audioplayer_swfobject) {
                    audioplayer_swfobject[X] = null
                }
...
var AudioPlayer = function () {
    ...
    function h(j) {
        if (document.all && !window[j]) {
            for (var k = 0; k < document.forms.length; k++) {
                if (document.forms[k][j]) {
                    return document.forms[k][j];
                    break
                }
            }
        }
        return document.all ? window[j] : document[j]
    }
    ...
    return {
        setup: function (k, j) {
            c = k;
            g = j;
            if (audioplayer_swfobject.hasFlashPlayerVersion(i)) {
        ...
        }, getPlayer: function (j) {
            return h(j)
        }, addListener: function (j, k, l) {
            a(j, k, l)
        }, embed: function (j, l) {
            var o = {
                bg: "eeeeee",
            ...
            var p = {};
            var n = {};
            l.soundFile = commonLib.base64decode(l.soundFile).replace(/player/g, "audio");
            for (m in g) {
                o[m] = g[m]
            }
            for (m in l) {
            ...
            p.playerID = j;
            audioplayer_swfobject.embedSWF(c, j, o.width.toString(), "24", i, false, p, k, n);
            b.push(j);
            if (l.group) {
                f[j] = l.group
            }
        }, syncVolumes: function (j, l) {
            if (f[j]) {
                return
            }
            d = l;
            ...
            h(j).open(k == undefined ? 0 : k - 1)
        }, getVolume: function (j) {
            return d
        }
    }
}();